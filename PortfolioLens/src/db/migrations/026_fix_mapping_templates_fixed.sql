...
DROP POLICY IF EXISTS "mapping_templates_select_policy" ON public.mapping_templates;
DROP POLICY IF EXISTS "mapping_templates_insert_policy" ON public.mapping_templates;
DROP POLICY IF EXISTS "mapping_templates_update_policy" ON public.mapping_templates;
DROP POLICY IF EXISTS "mapping_templates_delete_policy" ON public.mapping_templates;
...
CREATE POLICY "mapping_templates_insert_policy" 
ON public.mapping_templates
FOR INSERT 
WITH CHECK (auth.uid() IS NOT NULL);

-- For UPDATE: Users can only update their own templates or admin users can update any
CREATE POLICY "mapping_templates_update_policy" 
ON public.mapping_templates
FOR UPDATE 
USING (
...
CREATE POLICY "mapping_templates_delete_policy" 
ON public.mapping_templates
FOR DELETE 
USING (
  (owner_id = auth.uid() OR created_by = auth.uid()::text) OR
...
DROP VIEW IF EXISTS mapping_templates_view;
CREATE VIEW mapping_templates_view AS
SELECT 
