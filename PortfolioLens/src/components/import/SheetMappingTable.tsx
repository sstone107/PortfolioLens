...
export const SheetMappingTable: React.FC<SheetMappingTableProps> = ({
  ...
  const storeSheets = useBatchImportStore(state => state.sheets);
  ...
  useEffect(() => {
    ...
    Object.entries(storeSheets).forEach(([name, sheetState]) => {
  ...
  }, [storeSheets, storeUpdateNewTableName]);
  ...
  const handleNewTableNameBlur = (sheetName: string) => {
    ...
    const currentSheetState = storeSheets[sheetName];
    if (newName !== undefined && currentSheetState && currentSheetState.isNewTable) {
                        ...
                        <FormControl fullWidth size="small" sx={{ mt: 0 }}>
                          <Select
                            ...
                            onChange={(e) => {
                              ...
                              if (val === 'CREATE_NEW_TABLE_SENTINEL') {
                                const newName = toSqlFriendlyName(sheetName);
                                onTableSelection(sheetName, `new:${newName}`);
                                setEditableNewTableNames(prev => ({ ...prev, [sheetName]: newName }));
                              } else {
                                onTableSelection(sheetName, val);
                                if (val.startsWith('new:')) {
                                  setEditableNewTableNames(prev => ({ ...prev, [sheetName]: val.substring(4) }));
                                } else {
                                  setEditableNewTableNames(prev => {
                                    const updated = { ...prev };
                                    if (Object.prototype.hasOwnProperty.call(updated, sheetName)) {
