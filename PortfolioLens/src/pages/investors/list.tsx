...
// Define the investor interface with strict typing
interface Investor {
...
/**
 * Investor list component for displaying all investors in a data grid
 * with sorting, filtering, and pagination
 */
export const InvestorList: React.FC<IResourceComponentsProps> = () => {
  const { dataGridProps } = useDataGrid<Investor>({
    syncWithLocation: true,
  });

  // Define columns for the data grid with appropriate types and formatting
  const columns = React.useMemo<GridColDef<Investor>[]>(
    () => [
